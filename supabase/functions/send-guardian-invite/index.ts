...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { student_id, invitee_email } = await req.json();
    ...
    // Send invite email via Supabase Auth admin (generates a magic link email)
    // We'll use the admin API to send a custom email invitation
...
<body style="margin:0;padding:0;background-color:#f4f4f5;font-family:Arial,sans-serif;">
  <div style="max-width:560px;margin:40px auto;background:#ffffff;border-radius:12px;padding:40px;border:1px solid #e4e4e7;">
    ...
    // Send email using Supabase Auth admin invite (or direct SMTP)
    // Use the admin API to send to the invitee
    ...
    // If the user already exists, the invite email won't send via auth.
    // In that case, we still have the invite link stored — the parent can share it manually.
