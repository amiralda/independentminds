    ...
    <Dialog open={open} onOpenChange={v => { if (!v) resetAndClose(); }}>
      <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <p className="text-[11px] text-muted-foreground mb-3">
          {t("student.stepOf").replace("{{step}}", String(step))}
          {" — "}
          {step === 1 && t("student.personalInfo")}
          {step === 2 && t("student.scheduleDocs")}
          {step === 3 && t("student.reviewSubmit")}
        </p>
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-4 mb-2">
              ...
              <div className="text-xs text-muted-foreground">
                <p className="font-medium text-foreground">{t("profile.profilePhoto")}</p>
                <p>{t("profile.photoOptional")}</p>
              </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <label className="text-sm font-medium">{t("student.dob")}</label>
            ...
            <Button onClick={() => setStep(2)} disabled={!canProceedStep1} className="w-full font-display">
              {t("action.continue")} <ArrowRight size={16} className="ml-2" />
            </Button>
          </div>
        )}
          ...
          <div className="space-y-3">
            <div className="rounded-lg border-2 border-dashed border-muted-foreground/30 p-4 text-center">
              ...
              <p className="text-sm font-medium">
                {t("student.uploadSchedule")}
              </p>
              <p className="text-[11px] text-muted-foreground mt-1 mb-3">
                {t("student.uploadScheduleDesc")}
              </p>
              <Button variant="outline" size="sm" onClick={() => scheduleInputRef.current?.click()} disabled={extracting}>
                {extracting ? (
                  <><Loader2 size={14} className="mr-2 animate-spin" /> {t("student.extracting")}</>
                ) : (
                  <><FileText size={14} className="mr-2" /> {t("student.chooseFile")}</>
                )}
              </Button>
              <input
            ...
            {extractedSchedule.length > 0 && (
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <p className="text-sm font-medium">{t("student.extractedSchedule")}</p>
                  <Button variant="ghost" size="sm" onClick={addScheduleRow}>+ {t("student.addRow")}</Button>
                </div>
                ...
                {!scheduleValidated && (
                  <Button variant="outline" size="sm" className="w-full" onClick={() => setScheduleValidated(true)}>
                    <Check size={14} className="mr-2" /> {t("student.validateSchedule")}
                  </Button>
                )}
                {scheduleValidated && (
                  <p className="text-xs text-primary flex items-center gap-1"><Check size={12} /> {t("student.scheduleValidated")}</p>
                )}
              </div>
            )}
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <Button onClick={() => setStep(3)} className="font-display">
                {t("action.continue")} <ArrowRight size={16} className="ml-1" />
              </Button>
            </div>
          </div>
        )}
        ...
        {step === 3 && (
          <div className="space-y-3">
            <div className="rounded-xl bg-muted/50 p-4 space-y-2">
              ...
              {dob && <p className="text-xs"><span className="text-muted-foreground">{t("student.dob")}:</span> {dob}</p>}
              {nationality && <p className="text-xs"><span className="text-muted-foreground">{t("profile.nationality")}:</span> {nationality}</p>}
              {address && <p className="text-xs"><span className="text-muted-foreground">{t("profile.address")}:</span> {address}</p>}
            </div>

            {extractedSchedule.length > 0 && (
              <div className="rounded-xl bg-muted/50 p-3">
                <p className="text-xs font-medium mb-1">{t("nav.schedule")} ({extractedSchedule.length} {t("rewards.block")})</p>
            ...
            <div className="rounded-lg bg-muted/50 p-3">
              <p className="text-xs font-medium mb-1">{t("student.defaultTracks")}</p>
              <ul className="text-[11px] text-muted-foreground space-y-0.5">
