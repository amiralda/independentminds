    ...
    <>
      {!open && (
        <button
          ...
          className="fixed bottom-4 left-4 z-40 flex items-center gap-2 rounded-full bg-amber-500 px-4 py-2.5 text-sm font-medium text-white shadow-lg hover:bg-amber-600 transition-colors"
                ...
                <div className="flex gap-1 justify-center">
                  {[1, 2, 3, 4, 5].map((n) => (
                    <button
                      key={n}
                      onClick={() => setRating(n)}
                      className={`text-lg ${n <= rating ? 'text-amber-500' : 'text-muted-foreground/30'}`}
                    >
                      ★
                    </button>
                  ))}
